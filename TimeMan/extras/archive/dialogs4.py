...
class DataWindow(wx.Frame):
    def __init__(self, parent, id, title):
        wx.Frame.__init__(self, parent, id, title, size=(870, 470))#(285,320))#
...
class BackDateDlg(wx.Dialog):
    def __init__(self, parent, id, title, projects):
        ...
        self.mins = wx.SpinCtrl(self.panel, -1, "", pos=(170, 80),
                                size=(40, -1)
                                )
...
################################################################################
    ...
    def InsertBackDate(self, name, date, days, mths, yrs, hrs, work_time ,maxx,
                       delta
                       ):
            ...
            if delta == 1:
                ...
                if hrs[maxx - 1] != 0.00:
                    dlg = wx.MessageDialog(self, '%d/%d/%d already has %4.2f' +
                                           'hrs logged.\n\nAre you sure you ' +
                                           'want to log a further %4.2fhrs?'
                                           %tuple(date + [hrs[maxx-1],
                                                          work_time
                                                          ]
                                                  ),
...
class WorkSessionDlg(wx.Dialog):
    def __init__(self, parent, id, title, projects):
        ...
        self.start_btn = wx.Button(self.panel, -1, 'Start Session',
                                   pos=(10, 80)
                                   )
...
#            time.sleep(20)            
            ...
            ths_wk = self.parent.proj_list.GetItemText(pos,2).split(":")
            #ths_wk = int(ths_wk[0]) + int(ths_wk[1]) / 60.
            
            #Change total times from decimal hours to time format
            tot_f = "%d:%2d" %(int(tot), int(round((tot%1) * 60)))
            tot_f = tot_f.replace(" ", "0")
            
            self.parent.proj_list.SetStringItem(pos,1,str(sum(hours)))
            self.parent.proj_list.SetStringItem(pos,2,str(hours_worked + ths_wk))

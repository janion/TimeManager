...
class Window(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        self.proj_list = wx.ListCtrl(self.panel,-1,pos=(10,40),size=(455,260),
                                     style=wx.LC_REPORT
                                     )
        #Add columns
        ...
        self.Bind(wx.EVT_LIST_ITEM_DESELECTED,self.OnDeselection,self.proj_list)
...
################################################################################
    ...
    def BackDate(self,event): #Add time entries without real time recording
        ...
        dlg = dialogs.BackDateDlg(self,-1,'Back date project', self.projects)
        dlg.ShowModal()
...
################################################################################
    ...
    def GetProjectInfo(self,item): #Find the relevant data from .csv files
        ...
        if self.proj_list.FindItem(-1,item) == -1:
            ...
            self.PopulateList(item, str(tot), str(this_week),
...
################################################################################
    ...
    def NewProject(self,event): #Create new project
        ...
        dlg = dialogs.NewProjectDlg(self,-1,'New Project')
        dlg.ShowModal()
        dlg.Destroy()
...
################################################################################
    ...
    def OpenData(self,event): #Show user data for the selected project
        #Open data dialog
        data_window = dialogs.DataWindow(
...
################################################################################
    ...
    def PopulateList(self, proj,tot,this_week,start,tot_days,index): #Add data
...
################################################################################
    ...
    def StartWork(self,event): #Allow user to record a work session
        ...
        dlg = dialogs.WorkSessionDlg(self,-1,'Work session', self.projects)
        dlg.ShowModal()
...
"""
Project / Total time / Time this week / Date started / Duration days

#This has been bodged away but not an elegant solution
-if only entry is today then when a session is recorded only [0] is written
    -if only entry is today and new file then when backdated today
        then [0] is written in extra row
###
        
-change time to hr:min not decimal
"""
